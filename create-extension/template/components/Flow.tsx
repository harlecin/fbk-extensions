...
/**
 * `__ID__/flow` — a WEB-ONLY extension component.
 *
...
export function Flow({ props }: { props: unknown }) {
  ...
  return (
    ...
    >
      ...
      */}
      <style>{`
        @keyframes __ID__-flow-in {
          from { opacity: 0; transform: translateY(12px); }
        }
        .reveal .slides section.present .__ID__-flow-stage {
          animation: __ID__-flow-in 420ms cubic-bezier(0.16, 1, 0.3, 1) both;
        }
        html[data-still] .reveal .slides section.present .__ID__-flow-stage {
          animation: none;
        }
        @media (prefers-reduced-motion: reduce) {
          .reveal .slides section.present .__ID__-flow-stage { animation: none; }
        }
      `}</style>
    </div>
  );
