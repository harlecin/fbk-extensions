# Acme Slate — a worked Folienbaukasten extension

This repository is a **complete, building, rendering example** of an
extension for the Folienbaukasten slide app: a theme (`Acme Slate`), one
PowerPoint-safe slide component (`acme/pricing-sheet`), one web-only slide
component (`acme/flow`), an LLM guide, and two starter decks.
...
> This directory is the worked example; the rules it demonstrates are written
> out in [the repository README](../README.md), and
> [`../toolkit`](../toolkit/README.md) is what builds it.

---

## What an extension is

One file — `acme-1.0.0.fbkset`, a zip — that a user double-clicks to
...
npm run pack                 # → dist/acme-1.0.0.fbkset (checks run first)
```
...
node ../toolkit/bin/fbk-pack.ts pack . --out dist --publisher "Acme GmbH"
...
- **`kind` is namespaced by the pack id.** `acme/pricing-sheet`. The packer
...
```ts
components: { 'acme/pricing-sheet': { component: PricingSheet } }
```
...
once per slide", "`acme/flow` needs Web Mode — if the deck is in PowerPoint
